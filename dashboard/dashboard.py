...
def dashboard_print_estimation_times(df_monitor, cfg, experiment_path):
    ...
    st.write(f"Number of steps: **{steps_number:_}** / **{training_steps:_}**, this is **{np.round(steps_number / training_steps * 100, 3)}**% of traning")


    st.write(":heavy_minus_sign:" * 34)
    st.write("### Estimations:")
    st.write(f"Estimation of hour when training ends: **{(time_starts + full_training_time).strftime('%H:%M:%S %Y-%m-%d')}**")
    st.write(f"Estimation time when training ends: **{_format_timedelta(full_training_time - delta)}**")
    st.write(f"Estimation of TOTAL training time: **{_format_timedelta(full_training_time)}**")
...
def main(cfg: DictConfig):
    ...
    with col1:
        env_name = st.selectbox("Select Environment Name", _list_directories(output_folder))

    with col2:
        algo_id = st.selectbox("Select Algorithm ID", _list_directories(os.path.join(output_folder, env_name)))

    with col3:
        experiment_name = st.selectbox("Select Experiment Name",
                                       _list_directories(os.path.join(output_folder, env_name, algo_id)))

    if st.button("Refresh experiment"):
        # Read logger
        experiment_path = os.path.join(output_folder, env_name, algo_id, experiment_name)
